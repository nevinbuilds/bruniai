...
export async function analyzeImagesWithVision(base_screenshot, pr_screenshot, diff_image, base_url, preview_url, pr_number, repository, sections_analysis, user_id) {
    ...
    try {
        ...
        const baseAnalysis = await analyzeBaseUrlAgent(stagehand, base_url, sections_analysis);
        // Agent 2: Analyze preview URL and compare with base.
        const previewAnalysis = await analyzePreviewUrlAgent(stagehand, preview_url, baseAnalysis, sections_analysis);
        // Agent 3: Analyze screenshot images for visual differences.
        const imageAnalysis = await analyzeImagesAgent(stagehand, base_screenshot, pr_screenshot, diff_image, base_url, preview_url, sections_analysis);
        // Combine all agent results.
        const combinedResult = combineAgentResults(baseAnalysis, previewAnalysis, imageAnalysis);
        // Fill in metadata.
...
export async function analyzeImagesWithVisionImageMode(base_screenshot, pr_screenshot, diff_image, base_url, preview_url, pr_number, repository, sections_analysis, user_id) {
    ...
    try {
        ...
        const baseAnalysis = createBaseAnalysisFromSections(sections_analysis || "");
        // Agent 2: Analyze preview URL and compare with base.
 ...
 */
function createBaseAnalysisFromSections(sectionsAnalysis) {
    const sections = [];
    ...
    return {
        ...
        layout_notes: "Analysis based on base screenshot visual sections (AI-detected)",
    };
}
