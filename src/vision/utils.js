 ...
 */
export function sanitizePrInput(text, maxLength = MAX_DESCRIPTION_LENGTH) {
    if (!text) {
        return "";
    }
    // Detect and log suspicious patterns
    if (detectSuspiciousPatterns(text)) {
        console.warn("Suspicious pattern detected in PR input. Sanitizing and proceeding.");
    }
    let sanitized = text;
 ...
 */
export function validateAndFixEnums(data) {
    const validStatusEnum = ["pass", "fail", "warning", "none"];
    const validCriticalIssuesEnum = [
        "none",
        "missing_sections",
        "other_issues",
    ];
    const validVisualChangesEnum = ["none", "minor", "significant"];
    const validRecommendationEnum = [
        "pass",
        "review_required",
        "reject",
    ];
    const validSectionStatus = ["Present", "Missing"];
    // Validate and fix status_enum
    if (!data.status_enum || !validStatusEnum.includes(data.status_enum)) {
        console.warn(`Invalid status_enum: ${data.status_enum}, defaulting to 'warning'`);
    ...
    if (!data.critical_issues_enum ||
        !validCriticalIssuesEnum.includes(data.critical_issues_enum)) {
        console.warn(`Invalid critical_issues_enum: ${data.critical_issues_enum}, defaulting to 'other_issues'`);
        data.critical_issues_enum = "other_issues";
    }
    // Validate and fix visual_changes_enum
    if (!data.visual_changes_enum ||
        !validVisualChangesEnum.includes(data.visual_changes_enum)) {
        console.warn(`Invalid visual_changes_enum: ${data.visual_changes_enum}, defaulting to 'minor'`);
        data.visual_changes_enum = "minor";
    }
    // Validate and fix recommendation_enum
    if (!data.recommendation_enum ||
        !validRecommendationEnum.includes(data.recommendation_enum)) {
        console.warn(`Invalid recommendation_enum: ${data.recommendation_enum}, defaulting to 'review_required'`);
    ...
    if (data.critical_issues?.sections) {
        for (const section of data.critical_issues.sections) {
            if (!section.status ||
                !validSectionStatus.includes(section.status)) {
                console.warn(`Invalid section status: ${section.status}, defaulting to 'Present'`);
 ...
 */
export function createImageComparisonHtml(base_screenshot, pr_screenshot, diff_image, base_url, preview_url) {
...
export function createSectionDiffReviewHtml(cards, base_url, preview_url) {
