...
import { BaseUrlAnalysisResultSchema, PreviewUrlAnalysisResultSchema, ImageAnalysisResultSchema, SectionDiffExplanationsSchema, } from "./types.js";
...
function isUnusableSectionExplanation(explanation) {
 ...
 */
export async function analyzeBaseUrlAgent(stagehand, base_url, sections_analysis) {
  ...
  `;
    const result = await stagehand.extract(systemPrompt, BaseUrlAnalysisResultSchema, { page });
 ...
 */
export async function analyzePreviewUrlAgent(stagehand, preview_url, base_analysis, sections_analysis) {
  ...
  `;
    const result = await stagehand.extract(systemPrompt, PreviewUrlAnalysisResultSchema, { page });
    // Close the page to prevent context leakage.
    await page.close();
    return result;
}
 ...
 */
export async function analyzeImagesAgent(stagehand, base_screenshot, pr_screenshot, diff_image, base_url, preview_url, sections_analysis) {
    console.log(`\n${"=".repeat(50)}\n🔍 Agent 3: Analyzing Images\n${"=".repeat(50)}`);
    // Create HTML page with images.
    const comparisonHtml = createImageComparisonHtml(base_screenshot, pr_screenshot, diff_image, base_url, preview_url);
    ...
    const tempHtmlPath = join(__dirname, "..", "..", `temp-images-comparison-${uniqueId}.html`);
    ...
    `;
        ...
        try {
            ...
            throw new Error(`Failed to parse agent response as JSON: ${parseError instanceof Error ? parseError.message : String(parseError)}`);
        }
...
export async function analyzeSectionDiffExplanationsAgent(cards, base_url, preview_url) {
    if (cards.length === 0) {
        return [];
    }
  ...
  "sections": [
    {
      ...
      "explanation_confidence": 0.84
            ...
            const explanation = result.sections.find((section) => section.section_id === card.section_id);
            if (explanation && !isUnusableSectionExplanation(explanation.explanation)) {
                explanations.push(explanation);
            }
            else {
                console.warn(`Section explanation for ${card.section_id} was too generic or missing in model output.`);
            }
        }
        catch (error) {
