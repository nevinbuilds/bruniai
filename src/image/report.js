...
export function buildImageModeVisualAnalysis(input) {
    ...
    const problematicSections = sectionMatches.filter((section) => section.status === "problematic");
    const meanSimilarity = sectionMatches.length === 0
        ? 0
    ...
    const status = missingSections.length > 0
        ...
        : problematicSections.length > 0 || meanSimilarity < 0.85
            ? "warning"
            : "pass";
    const visualChangesEnum = problematicSections.length > 0
        ? "significant"
        : meanSimilarity < 0.85
            ? "minor"
            : "none";
    const recommendationEnum = status === "fail"
        ? "reject"
        : status === "warning"
            ? "review_required"
            : "pass";
    ...
    const orderedHighlights = sectionMatches
        .filter((section) => section.status === "missing" || section.status === "problematic")
        .slice(0, 5);
    return validateAndFixEnums({
        ...
        critical_issues_enum: missingSections.length > 0 ? "missing_sections" : "none",
        structural_analysis: {
            ...
            broken_layouts: missingSections.length > 0 || problematicSections.length > 0
        ...
        visual_changes: {
            diff_highlights: orderedHighlights.map((section) => `${section.name}: ${section.humanDescription}`),
            animation_issues: "No animation analysis in deterministic image mode.",
            conclusion: problematicSections.length > 0
        ...
        conclusion: {
            critical_issues: missingSections.length > 0
                ? "Some design sections could not be located in the webpage screenshot."
                : "No missing sections detected.",
            visual_changes: problematicSections.length > 0
