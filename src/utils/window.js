...
page, url, width = 1920, height = 1080) {
    ...
    if (url) {
        try {
            ...
            console.warn(`ensureViewportSize: networkidle wait failed for ${url}: ${error}`);
    ...
    if (viewportSize.width === 0) {
        console.warn(`Viewport width is 0 after navigation, resetting to ${width}x${height}`);
        page.setViewportSize(width, height);
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
page, stepPx = 800, delayMs = 500, maxSteps = 40) {
    // Start from the top.
    await page.evaluate(() => {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    for (let i = 0; i < maxSteps; i++) {
        const reachedEnd = await page.evaluate((step) => {
            ...
            const maxScroll = Math.max(doc.scrollHeight, doc.offsetHeight, body ? body.scrollHeight : 0, body ? body.offsetHeight : 0) - (win.innerHeight || doc.clientHeight || 0);
    ...
    try {
        await page.waitForFunction(() => {
            ...
            return visible.every((img) => {
                ...
                const looksLikePlaceholder = imageSrc.includes("/placeholder.svg") ||
