...
import { downloadImageToPng, trimImageToContent, extractVisualSections, matchVisualSections, formatMatchedSectionsAsAnalysis, buildImageModeVisualAnalysis, } from "../image/index.js";
import { analyzeSectionDiffExplanationsAgent } from "../vision/index.js";
import { join } from "path";
import sharp from "sharp";
function buildLlmUnavailableExplanation(reason) {
    return `${reason} Review the matched section crop and diff image directly for the exact visual change.`;
}
async function cropImageRegion(imagePath, outputPath, bounds) {
...
async function resizeImageToWidth(inputPath, outputPath, width) {
...
export async function performImageToImageComparison(options) {
    const { baseImageUrl, previewImageUrl, imagesDir, prNumber = "", repository = "", } = options;
    ...
    const sectionMatches = await matchVisualSections(baseScreenshotPath, previewScreenshotPath, sectionsResult.sections);
    const sectionsAnalysis = formatMatchedSectionsAsAnalysis(sectionsResult, sectionMatches);
    ...
    for (let index = 0; index < sectionMatches.length; index++) {
        ...
        const baseSectionPath = join(imagesDir, `base_screenshot_${pageSuffix}_section_${indexPrefix}${match.sectionId}.png`);
        ...
        if (match.matchedRange) {
            previewSectionPath = join(imagesDir, `preview_screenshot_${pageSuffix}_section_${indexPrefix}${match.sectionId}.png`);
            ...
            sectionDiffPath = join(imagesDir, `diff_${pageSuffix}_section_${indexPrefix}${match.sectionId}.png`);
    ...
    if (process.env.OPENAI_API_KEY) {
        ...
        if (explainableSections.length > 0) {
            try {
                const explanations = await analyzeSectionDiffExplanationsAgent(explainableSections, baseImageUrl, previewImageUrl);
                const explanationsById = new Map(explanations.map((explanation) => [explanation.section_id, explanation]));
                let acceptedCount = 0;
                ...
                const degradedSections = explainableSections.filter((section) => !explanationsById.has(section.section_id));
                if (degradedSections.length > 0) {
                    console.warn(`Section explanation agent returned ${acceptedCount}/${explainableSections.length} usable explanations. Falling back for: ${degradedSections
                        .map((section) => section.section_id)
                        .join(", ")}`);
                    for (const section of degradedSections) {
                        const match = sectionMatches.find((candidate) => candidate.sectionId === section.section_id);
                        if (!match) {
                            continue;
                        }
                        match.humanDescription = buildLlmUnavailableExplanation("The section was compared with the vision model, but it did not return a section-specific explanation.");
                ...
                for (const match of sectionMatches) {
                    ...
                    match.humanDescription = buildLlmUnavailableExplanation("The section was compared with the vision model, but the LLM explanation step failed.");
                    match.explanationConfidence = null;
        ...
        for (const match of sectionMatches) {
            ...
            match.humanDescription = buildLlmUnavailableExplanation("The section was compared with the vision model, but OPENAI_API_KEY is not set so no LLM explanation was generated.");
            match.explanationConfidence = null;
            match.explanationSource = "fallback_no_key";
        }
    }
    const sectionResults = sectionMatches.map((match) => {
        const artifacts = sectionArtifacts.get(match.sectionId);
        return {
            section_id: match.sectionId,
            name: match.name,
...
//# sourceMappingURL=image-image-core.js.map